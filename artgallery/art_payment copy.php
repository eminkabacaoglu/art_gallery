...
if (isset($_GET["paymentdelete"])) {
  $id=$_GET['art_id'];
  $result = $db->delete("art_payment","art_id",$_GET['art_id'] );
...
<?php }
   ...
   if (isset($_GET["paymentInsert"])) {?>
    <div class="card shadow mb-4">
     ...
     <?php  header("Location:art_payment.php?success=True"); } else {?>
      ...
      <form name="purchase_add" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <label>Eser Seç</label>
              <select onchange="getArtId()" class="form-control"  name="art_id" id="art_id" required>
                <option value="">Seçim Yapın</option>
                <?php
                
                $sql = $db->qSql("SELECT * FROM actions,arts WHERE actions.art_id=arts.art_id order by arts.art_id ASC");
        ...
        <div class="card-body">
          <?php if ($_GET["success"] == TRUE) {?>
            ...
            <div class="table-responsive">
                ...
                <div align="right">
                    <a href="?paymentInsert=true"><button class="btn btn-success">Tahsil Et</button></a>
                    <br><br>
                </div>
                    ...
                    <tbody>
                        ...
                        ?>
                            ...
                            <tr>
                                ...
                                <td width="50" align="right"><?php echo number_format($row["payment_price"], 0, ",", ".") ?>₺</td>
                                ...
                                <td  align="center"  width="5"><a id="delete_payment" onclick="deleteConfirm(<?php echo $row['art_id'] ?>)" href="#" class="btn btn-danger btn-circle btn-sm"><i class="far fa-trash-alt"></i></a></td>
...
/*  if(confirm("Lot-"+id+" Nolu Kayıt Silinecek Emin Misiniz?")){
  ...
  if (result.value) {
    ...
    document.getElementById("delete_payment").href="?paymentdelete=True&art_id="+id;
...
function getArtId() {
 var art_id2=document.getElementById("art_id").value;
  $.post('art_payment.php?paymentInsert=true',{art_id:art_id2},function(data){
    $('#payment_price').html(data)
    })
    <?php $art_id=$_POST['art_id']; $rec=$db->qSql("SELECT * FROM actions,members WHERE actions.art_id=$art_id and actions.member_id=members.member_id");$recs = $rec->fetch(PDO::FETCH_ASSOC);?>
    
    string="<option value='+<?php echo $recs['member_id']?>;+'>"+'<?php echo $recs['member_lastname']?>';+"</option>";

    document.getElementById("member_id").innerHTML=string;
