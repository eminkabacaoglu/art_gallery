...
if (isset($_POST["artActionInsert"])) {
  $result = $db->insert("actions", $_POST, [
    "form_name" => "actionInsert"
  ]);
...
<div class="form-popup" id="artsForm">
  ...
  <form method="POST" name="artAction" class="form-container-art">
    ...
    <div class="card-body">
      <form name="art_upd" method="POST" class="form-container" enctype="multipart/form-data">
        ...
        <div class="table-responsive">
          <table class="table table-bordered" id="dataTable2" width="100%" cellspacing="0">
            <thead>
              <tr>
                ...
                <th width="1" style="text-align:center"><button type="button" class="btn btn-success btn-circle btn-sm" onclick="get_data(event)"><i class="fas fa-plus"></i></button></th>
          ...
          <div align=center>
            <button type="submit" id="submit_action" class="btn2" name="artActionInsert">Kaydet</button>
...
<script>
  ...
  function get_data(event) {
    ...
    <?php
    $id = '<script>document.writeln(art_id)</script>';
    $sql2 = $db->qSql("SELECT * FROM arts, artists,arttype WHERE arts.artist_id=artists.artist_id AND arts.arttype_id=arttype.arttype_id AND arts.art_status='1' order by arts.art_id ");

    //$a_id = $row2["art_id"];

    
    ?>
    $('#addr' + i).html("<td ><select class='form-control input-md' id='art_id' name='art_id' required><?php while ($row2 = $sql2->fetch(PDO::FETCH_ASSOC)) {?><option value=<?php echo $row2['art_id'] ?>><?php echo "Lot-".$row2["art_id"]." ".$row2["art_name"]?></option><?php  } ?></select></td><td><input type='text' name='member_id' name='member_id' placeholder='Name' class='form-control input-md'/></td><td>Alış Fiyat</td><td><input type='text' id='sales_price' name='sales_price' placeholder='Fiyat' class='form-control input-md'/></td><td width='1' align='center'><button onclick='remove_line(event)' type='button' class='btn btn-danger btn-circle btn-sm'>X</button></td>");

    <?php
    $sql3 = $db->qSql("SELECT * FROM art_list_price WHERE art_list_price.art_id=1 ORDER BY list_price_id DESC LIMIT 0 , 1 ");
    $row3 = $sql3->fetch(PDO::FETCH_ASSOC)
    ?>
