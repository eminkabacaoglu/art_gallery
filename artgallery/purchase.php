...
if (isset($_GET["purchasedelete"])) {
  $id=$_GET['art_id'];
  $result = $db->delete("art_purchase","art_id",$_GET['art_id'] );
  ...
  if ($result["status"] == TRUE) { ?>
    ...
    <?php header("Location:purchase.php?success=True"); ?>
   ...
   if (isset($_GET["purchaseInsert"])) {?>
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        ...
        <?php 
        if (isset($_POST["purchase_insert"])) {

         $result=$db->insert("art_purchase",$_POST,[
          "form_name" => "purchase_insert","Id" => "art_id"
        ]
      );
     ...
     <?php  header("Location:purchase.php?success=True"); } else {?>
      ...
      <form name="purchase_add" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <label>Eser Seç</label>
              <select class="form-control"  name="art_id" id="art_id" required>
                ...
                while ($row=$sql->fetch(PDO::FETCH_ASSOC)) {
                    $id=$row['art_id'];  
                    $purchase_sql = $db->qSql("SELECT * FROM art_purchase where art_purchase.art_id=$id");
          ...
          <label>Alış Fiyat</label>
              ...
              <input onfocusout="validatePrice()"  class="form-control" type="text" name="purchase_price" id="purchase_price" required>

        </div>

        <div align="right" >
          <button type="submit" class="btn btn-success" name="purchase_insert">Ekle</button>
...
<div class="container-fluid" id="container-fluid">
  ...
  <p class="mb-4">Sanatçılar Liste<a target="_blank" href="https://datatables.net">official DataTables documentation</a>.</p> -->
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Alış</h6>
        </div>
        <div class="card-body">
          <?php if ($_GET["success"] == TRUE) {?>
            ...
            <div class="table-responsive">
                ...
                <div align="right">
                    <a href="?purchaseInsert=true"><button class="btn btn-success">Yeni Alış</button></a>
                    <br><br>
                </div>
            ...
            <tr>
              ...
              <th>Salary</th>
                    ...
                    <tbody>
                        ...
                        $sql = $db->qSql("SELECT * FROM arts, artists,arttype,art_purchase,members WHERE arts.artist_id=artists.artist_id AND arts.art_id=art_purchase.art_id AND arts.arttype_id=arttype.arttype_id AND members.member_id=art_purchase.member_id order by art_purchase.purchase_date DESC");
                        $count = 0;
                        while ($row = $sql->fetch(PDO::FETCH_ASSOC)) {
                            $count++
                        ?>
                            <?php $date = date_create($row["purchase_date"]); ?>
                            ...
                            <tr>
                                ...
                                <td width="70" align="right"><?php echo number_format($row["purchase_price"], 0, ",", ".") ?>₺</td>
                                ...
                                <td  align="center"  width="5"><a id="delete_purchase" onclick="deleteConfirm(<?php echo $row['art_id'] ?>)" href="#" class="btn btn-danger btn-circle btn-sm"><i class="far fa-trash-alt"></i></a></td>
...
/*  if(confirm("Lot-"+id+" Nolu Kayıt Silinecek Emin Misiniz?")){
  ...
  if (result.value) {
    ...
    document.getElementById("delete_purchase").href="?purchasedelete=True&art_id="+id;
    document.getElementById("delete_purchase").onclick="" //onclick fonksiyonu tekrar çalışmasın diye önce boşalıyoruz.
    document.getElementById("delete_purchase").click()})
  }
  else{
    Swal.fire(
      'İptal Edildi!',
...
function validatePrice() {
    ...
    //if (id_val == "updart_price" || id_val == "updart_purchase" || id_val == "updart_price") {
      if (isNaN(Number(document.getElementById("purchase_price").value))) {
