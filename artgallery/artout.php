      ...
      <form name="art_upd" method="POST" class="form-container" enctype="multipart/form-data">
        ...
        <div class="table-responsive">
          <table class="table table-bordered" id="dataTable2" width="100%" cellspacing="0">
            <thead>
              <tr>
                ...
                <th>Alış Fiyatı</th>
                <th>Satış Fiyatı</th>
                <th style="text-align:center"><button type="button" class="btn btn-success btn-circle btn-sm" onclick="open_arts()"><i class="fas fa-plus"></i></button></th>
          ...
          ?>
            <tbody id="art_list">
             <!--- <tr>
                <td width="2"><button type="button" onclick="open_arts()" class="btn btn-primary btn-circle btn-sm"><b>Seç</b> </button></td>
                <td width="200"><?php echo $row2["art_name"] ?></td>
                <td width="200"><?php echo $row2["artist_name"]." ".$row2["artist_lastname"]?></td>
                <td width="150"><?php echo $row3["list_price_amount"] ?></td>
                <td width="150"><input type="text"></td>
                <td width="1" align="center"><button onclick="remove_line(event)" type="button" class="btn btn-danger btn-circle btn-sm">X</button></td>
              </tr>--->
            </tbody>
          </table>
...
<script>
  ...
  function get_data(event) {
    ...
    $.post('artout.php',{art_id:art_id},function(data){
      $('#price').html(data)
    })
    <?php   
          $sql2 = $db->qSql("SELECT * FROM arts, artists,arttype WHERE arts.artist_id=artists.artist_id AND arts.arttype_id=arttype.arttype_id AND arts.art_status='1' AND arts.art_id=$_POST[art_id] order by arts.art_id ");
          $row2 = $sql2->fetch(PDO::FETCH_ASSOC);
          ...
          $row3 = $sql3->fetch(PDO::FETCH_ASSOC)
          ?>
           document.getElementById("art_list").innerHTML += `
           <tr>
               <!--- <td width="2"><button type="button" onclick="open_arts()" class="btn btn-primary btn-circle btn-sm"><b>Seç</b> </button></td>--->
                <td width="200"><?php echo $row2["art_name"] ?></td>
                <td width="200"><?php echo $row2["artist_name"]." ".$row2["artist_lastname"]?></td>
                <td width="150"><?php echo $row3["list_price_amount"] ?></td>
                <td width="150"><input id="price" type="text"></td>
                <td width="1" align="center"><button onclick="remove_line(event)" type="button" class="btn btn-danger btn-circle btn-sm">X</button></td>
              </tr>
    `
